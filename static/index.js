...
function shareText(score) {
    ...
    SubmitResults();
    if (score === 1) return '（一个人只能投一次奥）';
    if (score === 3) return '我是3号。';
    if (score === 114) return '哼哼哼啊啊啊啊啊啊啊啊';
    if (score === 129) return '星王生日捏';
    if (score <= 49) return '星王の不屑';
    if (score <= 99) return '不过如此';
    if (score <= 149) return '好好看';
    return '我不说就不算';
}
...
console.log("不修改，好嘛？乱传又有什么用呢？(ˉ▽ˉ；)...就要改 sy留")
